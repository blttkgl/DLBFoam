...
void Foam::binaryTree_pyJac::insertNewLeaf
(
    const scalarField& phiq,
...
)
{
    ...
    else // at least one point stored
    {
        ...
        binaryNode_pyJac* parentNode = phi0->node();
        ...
        // the hyper plane is computed in the binaryNode_pyJac constructor
        binaryNode_pyJac* newNode;
        if (size_>1)
        {
            newNode = new binaryNode_pyJac(phi0, newChemPoint, parentNode);
            // make the parent of phi0 point to the newly created node
            insertNode(phi0, newNode);
        }
        else // size_ == 1 (because not equal to 0)
        {
            // when size is 1, the binaryNode_pyJac is without hyperplane
            deleteDemandDrivenData(root_);
            newNode = new binaryNode_pyJac(phi0, newChemPoint, nullptr);
            root_ = newNode;
        }
...
bool Foam::binaryTree_pyJac::secondaryBTSearch
(
    const scalarField& phiq,
    chemPointISAT_pyJac*& x
)
{
    ...
    {
        chemPointISAT_pyJac* xS = chemPSibling(x);
        if (xS != nullptr)
        {
            n2ndSearch_++;
            if (xS->inEOA(phiq))
...
void Foam::binaryTree_pyJac::deleteLeaf(chemPointISAT_pyJac*& phi0)
{
    ...
    {
        binaryNode_pyJac* z = phi0->node();
        binaryNode_pyJac* x;
        chemPointISAT_pyJac* siblingPhi0 = chemPSibling(phi0);
...
void Foam::binaryTree_pyJac::balance()
{
    //1) walk through the entire tree by starting with the tree's most left
    // chemPoint
    chemPointISAT_pyJac* x = treeMin();
    List<chemPointISAT_pyJac*> chemPoints(size_);
    label chemPointISAT_pyJaci=0;
    while (x != nullptr)
    {
        chemPoints[chemPointISAT_pyJaci++] = x;
        x = treeSuccessor(x);
    }
    ...
    binaryNode_pyJac* newNode = new binaryNode_pyJac
    (
    ...
    for (label cpi=1; cpi<chemPoints.size()-1; cpi++)
    {
        ...
        // add the chemPoint
        binaryNode_pyJac* nodeToAdd = new binaryNode_pyJac
        (
            phi0,
...
Foam::chemPointISAT_pyJac* Foam::binaryTree_pyJac::treeSuccessor(chemPointISAT_pyJac* x)
{
    if (size_>1)
    {
